...
/// <summary>
/// Low-level keyboard hook so Win, Alt+Tab, etc. are forwarded to the remote rather than handled locally.
/// Only suppresses keys when Active = true (i.e. the desktop view is open and window is focused).
/// </summary>
public sealed class KeyboardHook : IDisposable
{
    public event Action<int, bool>? KeyEvent; // (virtualKey, isDown)
    ...
    private nint HookCallback(int nCode, nint wParam, nint lParam)
    {
        if (nCode >= 0 && Active)
        {
            ...
            if (isDown || isUp)
            {
                ...
                return 1; // Suppress — don't let Windows act on Win key, Alt+Tab, etc.
            }
        }

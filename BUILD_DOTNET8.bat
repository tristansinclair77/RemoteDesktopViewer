...
if not errorlevel 1 (
    ...
    echo Right-click the tray icon and choose Exit, then re-run this script.
    pause
    exit /b 1
)
tasklist /FI "IMAGENAME eq RDV.Viewer.exe" 2>NUL | find /I "RDV.Viewer.exe" >NUL
if not errorlevel 1 (
    echo ERROR: RDV.Viewer.exe is still running. Close it, then re-run this script.
    pause
    exit /b 1
)
...
echo [3/5] Publishing RDV.Host (self-contained .NET 8)...
...
echo [4/5] Publishing RDV.Viewer (self-contained .NET 8)...
